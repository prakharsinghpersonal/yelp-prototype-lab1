...
export default function AiChatWidget() {
  ...
  const handleSend = async (query) => {
    ...
    setMessages((prev) => [...prev, { role: 'user', content: text, restaurants: [] }])
    setLoading(true)
  ...
  return (
    <div className="fixed bottom-6 right-6 z-50 flex flex-col items-end">
      ...
      {isOpen && (
        <div className="bg-white rounded-2xl shadow-2xl border border-gray-200 w-[350px] sm:w-[400px] h-[600px] max-h-[80vh] flex flex-col overflow-hidden mb-4">
          {/* Header */}
          <div className="bg-[#003049] text-white p-4 flex items-center justify-between">
            <h3 className="font-bold flex items-center gap-2">
              <span className="w-8 h-8 rounded-full bg-white/20 flex items-center justify-center text-xl">🤖</span>
              AI Assistant
            </h3>
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-4 bg-gray-50">
            {messages.map((msg, i) => (
              <div key={i} className={`flex gap-2 ${msg.role === 'user' ? 'justify-end' : 'justify-start items-end'}`}>
                {msg.role === 'assistant' && (
                  <div className="w-7 h-7 rounded-full bg-[#003049] flex items-center justify-center text-white text-sm flex-shrink-0 mb-1">
                    🤖
                  </div>
                )}
                      ...
                      {msg.restaurants.map((r) => (
                        ...
                        >
                          ...
                          <div className="min-w-0">
                            ...
                            <p className="text-[10px] text-gray-500 truncate">{r.cuisine_type} · {r.city}</p>
                          </div>
                        </Link>
                      ))}
                    </div>
            ...
            {loading && (
              <div className="flex justify-start gap-2 items-end">
                <div className="w-7 h-7 rounded-full bg-[#003049] flex items-center justify-center text-white text-sm flex-shrink-0 mb-1">🤖</div>
              ...
              <div className="flex flex-wrap gap-1 mb-2">
                {QUICK_ACTIONS.map((a) => (
                  <button
                    ...
                    className="text-[10px] px-2 py-1 rounded-full border border-[#f77f00] text-[#f77f00] hover:bg-[#f77f00] hover:text-white transition-colors"
                  >
                    {a}
                  </button>
                ))}
      ...
      <button
        ...
        className={`${
          isOpen ? 'bg-gray-800 hover:bg-gray-900' : 'bg-[#003049] hover:bg-[#004a6e] hover:scale-105'
        } text-white rounded-full w-14 h-14 flex items-center justify-center shadow-xl transition-all duration-200`}
